...
TrustGuard RAG 是一个独立的知识库、检索与回答服务，目前负责三类核心业务：

1. 将 PDF、DOCX、文本、Markdown、CSV、JSON、HTML 和图片转化为可检索的知识分块。
2. 同时使用 Qdrant 向量召回和 OpenSearch BM25 召回，再经过融合及可选 Rerank 返回带来源信息的知识片段。
3. 将检索证据按 Token 预算组装为上下文，调用兼容 LLM 生成带可验证引用的回答，并在证据不足时拒答。

`POST /v1/search` 仍只负责检索；`POST /v1/answer` 在其上增加上下文组装、回答生成和引用校验。两条接口保持独立，便于上游 Agent 选择自行生成或直接复用完整回答。
...
| POST | `/v1/answer` | 复用混合检索，生成带引用的回答或明确拒答 |
