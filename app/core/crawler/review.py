...
def review_response(job_id: str, progress: dict[str, Any]) -> dict[str, Any]:
    ...
    pending = sum(item.get("status") in {"pending", "processing"} for item in items)
...
async def apply_review(
    ...
    *,
    ...
    items = [dict(item) for item in progress.get("review_items") or []]
    item_index = {str(item.get("id") or ""): item for item in items}
    unknown = set(item_ids) - set(item_index)
    if unknown:
        raise LookupError("Review item not found")
    ...
    for item_id in dict.fromkeys(item_ids):
        ...
        item["ingest_job_id"] = ingest_job.id
    ...
    await store.update_progress(job_id, progress=progress)
    return review_response(job_id, progress)
